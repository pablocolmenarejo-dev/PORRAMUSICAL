...
const RevealView = () => {
  ...
  // --- NUEVO: CÁLCULO DEL RANKING EN DIRECTO ---
  const liveScores = useMemo(() => {
    const scores = participants.map(participant => {
        const correctVotes = votes.filter(vote => {
            // El voto debe ser del participante actual
            if (vote.voterId !== participant.id) return false;
            
            // La canción votada debe estar entre las reveladas
            if (!revealedSongs.has(vote.songId)) return false;
  ...
  }, [revealedSongs, participants, songs, votes]);
                    ...
                    return (
                        <Card key={song.id} className={`!p-0 flex flex-col overflow-hidden bg-gray-800 transition-all duration-500 ${isRevealed ? 'border-2 border-green-500' : 'border-2 border-transparent'}`}>
                            ...
                            <div className="p-4 flex flex-col flex-grow">
                                ...
                                {!isRevealed && <Button onClick={() => handleReveal(song.id)} className="w-full">Revelar</Button>}
                                {isRevealed && (
